...
let private packTest (bom: bool) (hash: Sha1Hash) (source: string): Task = task {
...
let ``Packed commit should be extracted properly``(): Task =
    let commitHash = "8c3ecc6b9abdab719915046ce7e989715fde5f5b" |> Sha1Hash.OfString
    packTest false commitHash "commit"

[<Fact>]
let ``Packed tree should be extracted properly``(): Task =
    let treeHash = "8401dc8f768e38f83b4c2a5c73d5393b093f5e6c" |> Sha1Hash.OfString
    packTest false treeHash "tree"

[<Fact>]
let ``Packed blob should be extracted properly``(): Task =
    let blobHash = "25872623397735b9946b9e7ddaf48ea8292c6eb9" |> Sha1Hash.OfString
    packTest true blobHash "blob"

[<Fact>]
let ``Delta objects should be parsed and extracted properly``(): Task =
    let deltaObj = "d36f4ae4294cc19e91d8c8fab1fb816630a6d79b" |> Sha1Hash.OfString
