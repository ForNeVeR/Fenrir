...
let ``Packed commit should be extracted properly``(): Task =
    let commitHash = "8c3ecc6b9abdab719915046ce7e989715fde5f5b" |> Sha1Hash.OfHexString
    packTest false commitHash "commit"

[<Fact>]
let ``Packed tree should be extracted properly``(): Task =
    let treeHash = "8401dc8f768e38f83b4c2a5c73d5393b093f5e6c" |> Sha1Hash.OfHexString
    packTest false treeHash "tree"

[<Fact>]
let ``Packed blob should be extracted properly``(): Task =
    let blobHash = "25872623397735b9946b9e7ddaf48ea8292c6eb9" |> Sha1Hash.OfHexString
